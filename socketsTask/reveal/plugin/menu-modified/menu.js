...
var RevealMenu = window.RevealMenu || (function(){
	...
	loadResource(options.path + '/font-awesome-4.3.0/css/font-awesome.min.css', 'stylesheet', function() {
		...
		if (!bowser.msie || bowser.version >= 9) {
			...
			var buttons = 0;
			$('<li data-panel="Slides" data-button="' + (buttons++) + '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Слайды</span><br/><i class="fa fa-list"></i></li>')
				.appendTo(toolbar)
			...
			if (themes) {
				$('<li data-panel="Themes" data-button="' + (buttons++) + '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Темы</span><br/><i class="fa fa-desktop"></i></li>')
					.appendTo(toolbar)
					.click(openPanel);
			}
			if (transitions) {
				$('<li data-panel="Transitions" data-button="' + (buttons++) + '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Переходы</span><br/><i class="fa fa-arrows-h"></i></li>')
					.appendTo(toolbar)
					.click(openPanel);
			}
			$('<li id="close"><span class="slide-menu-toolbar-label">Закрыть</span><br/><i class="fa fa-times"></i></li>')
				.appendTo(toolbar)
				.click(closeMenu);
			...
			function openItem(item) {
				...
				if (typeof h !== "undefined" && typeof v !== "undefined") {
					...
					//#kontur: Allows to update all slides after change theme
					setTimeout(function () { Reveal.sync(); }, 100);
					closeMenu();
				} else if (transition) {
